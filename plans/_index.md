...
| 13 | [Foundations: Vocabulary, Numbers, Aksharmala](phase-13-foundations-vocab-numbers-alphabet.md) | 🚧 | Three new beginner content types, modeled on `learn-bharat`'s Fundamentals but built to avoid its live-TTS/dead-image/incomplete-alphabet mistakes. Started 2026-08-03 — project owner re-sequenced ahead of phase 12's remaining steps (i18n/app-store/tutor-verification/monetization), paused rather than finished first. Schema + Convex backend now built (6 new tables, `vocabulary.ts`, `aksharmala.ts`). Devanagari Aksharmala content authoring is done: all 49 characters seeded, reviewed, and `live`, with audio now on Google Chirp3-HD (`convex/google/aksharmalaTts.ts`) rather than Bhashini — see that file's header and `specs/data-model.md`'s scriptCharacterAudio note for why. Numbers/Family vocabulary content authoring (images + hi audio) is in progress, not yet promoted |
